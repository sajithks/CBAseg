...
#%% micro phase dataset
startime = time.time()
gt = cv2.imread('/Users/sajithks/Documents/project_cell_tracking/processed/result_comparisons/segmentation/dataset/johan_fluo/gt_elf_fluo.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
gt = cb.removeBoundaryLabel(gt)
gt = cb.labelDilation(gt,3)

#cbaout = cv2.imread('/Users/sajithks/Documents/project_cell_tracking/phase images from elf/fluoout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
cbaout = cv2.imread('cba_fluo_outimg.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
cbaout = cb.removeBoundaryLabel(cbaout)
cbaout = cb.labelDilation(cbaout,3)


cbafilt = cv2.imread('cba_fluo_filterout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
cbafilt = cb.removeBoundaryLabel(cbafilt)
cbafilt = cb.labelDilation(cbafilt,3)

cbasmooth = cv2.imread('cba_fluo_smoothout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
cbasmooth = cb.removeBoundaryLabel(cbasmooth)
cbasmooth = cb.labelDilation(cbasmooth,3)

mamle = cv2.imread('mamle_elf_fluo.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)

mamle = cb.removeBoundaryLabel(mamle)
#microb = cb.labelDilation(microb,3)
...
#recall4, precision4, fmeasure4, dicescore4 = cb.evaluateSegmentation2(gt, microb)
...
line1, = plt.plot(xval, ft1, label='CBA',color = [.2,.4,.6])
#line1, = plt.plot(xval, ft1, label='CBA',color = 'k')

line2, = plt.plot(xval, ft2, label='CBA + filtering',color = 'r', linestyle = '--')
line3, = plt.plot(xval, ft3, label='CBA + filtering + smoothing',color = 'g', linestyle = ':' ,marker = '')
...
#plt.xlabel('F-score fluorescent data ')#,plt.ylabel('% of total cells')
#plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)},loc='lower left')
#plt.show()
#plt.yticks(np.arange(0, 1.25, .25),[0,'',50,'',100])
...
plt.savefig(plotloc+'fluofscorecumul.png',bbox_inches='tight')
...
#plt.figure()#, plt.plot(xval, ft1, label='outimg','')#, xval, ft2, 'b', xval, ft3, 'r', xval, ft4, 'g'),plt.title('F-score Elf phase'),plt.legend(('line1','line2','line3','line4'),('outimg','filterout','smoothout','microbetracker')),plt.show()
#line1, = plt.plot(xval, r1, label='CBA',color = 'k')
#line2, = plt.plot(xval, r2, label='CBA + filtering',color = 'r', linestyle = '--')
#line3, = plt.plot(xval, r3, label='CBA + filtering + smoothing',color = 'g', linestyle = ':' ,marker = '')
#line4, = plt.plot(xval, r4, label='Microbetracker',color = 'b', linestyle = '-')
#line5, = plt.plot(xval, r5, label='MAMLE',color = 'y', linestyle = '-')
#
#plt.xlabel('Recall fluorescent data '),plt.ylabel('Number of cells')
##plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)},loc='upper right')
#frame=plt.gca()
#frame.axes.get_xaxis().set_visible(False)
#frame.axes.get_yaxis().set_visible(False)
#plt.savefig(plotloc+'fluorecall.png',bbox_inches='tight')
...
#
#plt.xlabel('Precision fluorescent data '),plt.ylabel('Number of cells')
##plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)})
##plt.show()
#frame=plt.gca()
#frame.axes.get_xaxis().set_visible(False)
#frame.axes.get_yaxis().set_visible(False)
#plt.savefig(plotloc+'fluoprec.png',bbox_inches='tight')
...
#
#plt.xlabel('Dice score fluorescent data '),plt.ylabel('Number of cells')
##plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)})
##plt.show()
#frame=plt.gca()
#frame.axes.get_xaxis().set_visible(False)
#frame.axes.get_yaxis().set_visible(False)
#plt.savefig(plotloc+'fluofscore.png',bbox_inches='tight')
...
microare =  sio.loadmat('/Users/sajithks/Documents/project_cell_tracking/processed/result_comparisons/segmentation/dataset/johan_fluo/microcontour/areamicrofluo_3.mat') 
...
plt.tick_params(axis='both', which='major', labelsize=30)
plt.savefig(plotloc+'fluoareadist.png',bbox_inches='tight')
