...
gt = cv2.imread('gt_labelmamle.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)

gt = cb.removeBoundaryLabel(gt)
gt = cb.labelDilation(gt,3)

cbaout = cv2.imread('cba_mamle_outimg.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
#cbaout = cv2.imread('/Users/sajithks/Documents/project_cell_tracking/phase images from elf/mamleout2.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
cbaout = cb.removeBoundaryLabel(cbaout)
cbaout = cb.labelDilation(cbaout,3)

cbafilt = cv2.imread('cba_mamle_filterout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
#cbafilt = cv2.imread('/Users/sajithks/Documents/project_cell_tracking/phase images from elf/mamlefiltout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
cbafilt = cb.removeBoundaryLabel(cbafilt)
cbafilt = cb.labelDilation(cbafilt,3)

cbasmooth = cv2.imread('cba_mamle_smoothout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
cbasmooth = cb.removeBoundaryLabel(cbasmooth)
cbasmooth = cb.labelDilation(cbasmooth,3)

mamle = cv2.imread('mamle_maleout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
#mamle = cv2.imread('/Users/sajithks/Documents/project_cell_tracking/processed/result_comparisons/segmentation/others/mamle_results/mamleout.tif',cv2.CV_LOAD_IMAGE_UNCHANGED)
...
#plotloc = '/Users/sajithks/Documents/project_cell_tracking/phase images from elf/ecoli_det_seg/ecoli_paper/images/metrics/'
plotloc = ''


matfile = sio.loadmat('phase2.mat')
...
#plt.xlabel('F-score phase data 3', fontsize=FONTSIZE),plt.ylabel('% of total cells', fontsize=FONTSIZE)
...
plt.savefig(plotloc+'phasdat2fscorecumul.png',bbox_inches='tight')
...
#
#plt.xlabel('Recall phase data 3', fontsize=FONTSIZE),plt.ylabel('Number of cells', fontsize=FONTSIZE)
##plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)})
##plt.show()
#frame=plt.gca()
#frame.axes.get_xaxis().set_visible(False)
#frame.axes.get_yaxis().set_visible(False)
#plt.savefig(plotloc+'phasdat2recall.png',bbox_inches='tight')
...
#
#plt.xlabel('Precision phase data 3', fontsize=FONTSIZE),plt.ylabel('Number of cells', fontsize=FONTSIZE)
##plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)})
##plt.show()
#frame=plt.gca()
#frame.axes.get_xaxis().set_visible(False)
#frame.axes.get_yaxis().set_visible(False)
#plt.savefig(plotloc+'phasdat2prec.png',bbox_inches='tight')
...
#
#plt.xlabel('Dice score phase data 3', fontsize=FONTSIZE),plt.ylabel('Number of cells', fontsize=FONTSIZE)
##plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)})
##plt.show()
#frame=plt.gca()
#frame.axes.get_xaxis().set_visible(False)
#frame.axes.get_yaxis().set_visible(False)
#plt.savefig(plotloc+'phasdat2fscore.png',bbox_inches='tight')
...
microare =  sio.loadmat( '/Users/sajithks/Documents/project_cell_tracking/processed/result_comparisons/segmentation/dataset/mamle/microcontour/area1.mat') 
...
#plt.xlabel('Cell area phase data 3', fontsize=FONTSIZE),plt.ylabel('% of cells ', fontsize=FONTSIZE)
#plt.legend(handler_map={line1: HandlerLine2D(numpoints=16)},loc='upper right')
#plt.show()
#frame=plt.gca()
plt.yticks(np.arange(0, .4, .1),np.arange(0, 40, 10))
plt.xticks(np.arange(0, 1250, 500))
plt.tick_params(axis='both', which='major', labelsize=30)

#frame.axes.get_xaxis().set_visible(False)
#frame.axes.get_yaxis().set_visible(False)
plt.savefig(plotloc+'phasdat2areadist.png',bbox_inches='tight')
